...
use crate::{
    amm::{
        pdas::{
            ...
            find_protocol_config_pda, find_protocol_treasury_pda, find_vault_a_pda,
            find_vault_b_pda,
        },
...
pub fn add_initial_liquidity_ix(
    ...
    let provider_token_a_ata =
        get_associated_token_address(provider, &initialized_pool.mint_a);
    let provider_token_b_ata =
        get_associated_token_address(provider, &initialized_pool.mint_b);
    let provider_lp_token =
        get_associated_token_address(provider, &initialized_pool.lp_mint);

    let accounts = accounts::AddInitialLiquidity {
        provider: *provider,
        pool: initialized_pool.pool,
        mint_a: initialized_pool.mint_a,
        mint_b: initialized_pool.mint_b,
        lp_mint: initialized_pool.lp_mint,
        vault_a: initialized_pool.vault_a,
        vault_b: initialized_pool.vault_b,
        provider_token_a_ata,
        provider_token_b_ata,
        locked_lp_token: initialized_pool.locked_lp_token,
        provider_lp_token,
        system_program: system_program::ID,
        token_program: token::ID,
        associated_token_program: associated_token::ID,
...
pub fn add_liquidity_ix(
    ...
    let program_id = &ctx.program_id;
    let provider_token_a = get_associated_token_address(provider, &initialized_pool.mint_a);
    let provider_token_b = get_associated_token_address(provider, &initialized_pool.mint_b);
    let provider_lp = get_associated_token_address(provider, &initialized_pool.lp_mint);
...
pub fn swap_ix(
    ...
    let accounts = accounts::Swap {
        ...
        protocol_config,
        protocol_treasury,
        treasury_a: initialized_pool.treasury_a,
        treasury_b: initialized_pool.treasury_b,
        system_program: system_program::ID,
        token_program: token::ID,
        associated_token_program: associated_token::ID,
    }
